...
architecture_config = [
    (7, 64, 2, 3), #Tuple : (kernel size,num_of_filters,stride,padding)
    "M",
    (3, 192, 1, 1),
    "M",
    (1, 128, 1, 0),
    ...
    (3, 512, 1, 1),
    "M",
    [(1, 256, 1, 0), (3, 512, 1, 1), 4], #List : (Tuple , number of repeats)
    (1, 512, 1, 0),
    (3, 1024, 1, 1),
    "M",
...
class CNNBlock(nn.Module):
    ...
    def __init__(self,in_channel,out_channels,**kwargs):
        super(CNNBlock,self).__init__()
        self.conv = nn.Conv2d(in_channel,out_channels,bias=False,**kwargs)
        self.batchnorn = nn.BatchNorm2d(out_channels)
        self.leakyrelu = nn.LeakyReLU(0.1)
    
    def forward(self,x):
        return self.leakyrelu(self.batchnorn(self.conv(x)))
...
class Yolov1(nn.Module):
    ...
    def _create_conv_layers(self,architecture):
        ...
        return nn.Sequential(*layers) #*layers [type == LIST] unpacks the list and converts it to an Sequential Model
